		...
		<div class="row">
			...
			<div class="col-md-10">
				...
				<h3 style="line-height: 2;">
					<span style="font-weight: bold">Họ tên: </span>${user.getName()}</h3>
				<h3 style="line-height: 2;">
					<span style="font-weight: bold">Số điện thoại: </span>${user.getTel()}</h3>
				<h3 style="line-height: 2;">
					<span style="font-weight: bold">Email: </span>${user.getEmail()}</h3>
				<h3 style="line-height: 2;">
					<span style="font-weight: bold"> Địa chỉ: </span>${user.getAddress()}</h3>
					...
					<c:forEach var="order" items="${list}" varStatus="loop">
						<tr style="text-align: center;">
							...
							<td><c:forEach var="od"
									...
									<p>
										<a href='<c:url value="/product?id=${od.product.id}" />'>${od.product.name}</a><br>
									</p>

									<c:choose>
										<c:when 
											test='${(order.status == "Đang chờ giao") || (order.status =="Đang giao")  }'>

											<p>Số lượng: ${od.orderQuantity }</p>
											<hr>
											<c:set var="tongGiaTri"
												value="${tongGiaTri + od.orderQuantity*od.price}" />
										</c:when>
										<c:otherwise >
											<p>Số lượng: ${od.receiveQuantity }</p>
											<hr>
											<c:set var="tongGiaTri"
												value="${tongGiaTri + od.receiveQuantity*od.price}" />
										</c:otherwise>
									</c:choose>
								</c:forEach></td>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<textarea rows="3" cols="1" class="form-control" id="address">${user.getAddress()}</textarea>
					</div>
